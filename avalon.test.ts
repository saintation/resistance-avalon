...
test.describe.serial('레지스탕스 아발론 자동화 봇 시뮬레이션 (v1.3 최종 안정화)', () => {
  ...
  test('1. 방 생성 및 전체 플레이어 입장', async () => {
    ...
    const titleText = await initialHostPage.locator('#roomTitle').innerText();
    ...
    await expect(initialHostPage.locator('#playerList .player-item')).toHaveCount(TOTAL_PLAYERS, { timeout: 10000 });
  });
  ...
  test('2. 전체 게임 사이클 무작위 시뮬레이션 및 UI 검증', async () => {
    ...
    for (let gameRound = 1; gameRound <= 3; gameRound++) {
      ...
      for (const p of pages) {
        if (await p.locator('#hostSettings').isVisible()) { currentHostPage = p; break; }
      }
      ...
      if (Math.random() > 0.5 && currentGood < goodMax) { await currentHostPage.check('#cbPercival', { force: true }); currentGood++; }
      else { await currentHostPage.uncheck('#cbPercival', { force: true }); }
      if (Math.random() > 0.5 && currentEvil < evilMax) { await currentHostPage.check('#cbMorgana', { force: true }); currentEvil++; }
      else { await currentHostPage.uncheck('#cbMorgana', { force: true }); }
      if (Math.random() > 0.5 && currentEvil < evilMax) { await currentHostPage.check('#cbMordred', { force: true }); currentEvil++; }
      else { await currentHostPage.uncheck('#cbMordred', { force: true }); }
      if (Math.random() > 0.5 && currentEvil < evilMax) { await currentHostPage.check('#cbOberon', { force: true }); currentEvil++; }
      else { await currentHostPage.uncheck('#cbOberon', { force: true }); }

      await expect(currentHostPage.locator('#startGameBtn')).not.toBeDisabled({ timeout: 15000 });
      
      await expect(async () => {
        if (await currentHostPage.locator('#waitingArea').isVisible()) {
          await currentHostPage.locator('#startGameBtn').click({ force: true });
        }
        await expect(currentHostPage.locator('#playArea')).toBeVisible({ timeout: 3000 });
      ...
      while (!isGameOver) {
        ...
        if (await pages[0].locator('#gameOverArea').isVisible()) {
          isGameOver = true;
          const reason = await pages[0].locator('#gameOverReason').innerText();
          console.log(`\n🏆 [게임 ${gameRound}회차 최종 결과] ${reason}\n`);
          break;
        }
        
        if (await pages[0].locator('#voteResultArea').isVisible()) {
          const voteMsg = await pages[0].locator('#voteResultMessage').innerText();
          console.log(`   👉 [팀 찬반 투표 완료] ${voteMsg.includes('가결') ? '🟢 가결' : '🔴 부결'}`);
          
          // 방장 상태 전환 락(Lock) 해제 대기 (데드락 방지용)
          ...
          await expect(async () => {
            ...
            await expect(pages[0].locator('#voteResultArea')).toHaveClass(/hidden/, { timeout: 1500 });
        ...
        if (await pages[0].locator('#questResultArea').isVisible()) {
          const msg = await pages[0].locator('#questResultMessage').innerText();
          ...
          await expect(async () => {
            ...
            await expect(pages[0].locator('#questResultArea')).toHaveClass(/hidden/, { timeout: 1500 });
          }).toPass({ intervals: [1000, 2000], timeout: 30000 });
        ...
        if (needsTeamVote) {
          ...
          await expect(async () => {
            ...
            await expect(pages[0].locator('#teamVotingArea')).toHaveClass(/hidden/, { timeout: 1500 });
          }).toPass({ intervals: [1000, 2000], timeout: 30000 });
        ...
        if (needsQuestVote) {
          ...
          await expect(async () => {
            for (let i = 0; i < pages.length; i++) {
              const p = pages[i];
              const nickname = i === 0 ? '방장' : `플레이어${i}`;
              const myRole = roleMap[nickname] || "";
              
              const isEvil = ["암살자", "모르가나", "모드레드", "모드레드의 수하", "오베론"].some(r => myRole.includes(r));
              ...
              if (await p.locator('#questVotingArea').isVisible() && await successBtn.isEnabled()) {
                // 악팀일 경우에만 일정 확률로 실패 투표
                if (isEvil && Math.random() < 0.3) {
                  await failBtn.click({ force: true }).catch(()=>{});
                } else {
                  await successBtn.click({ force: true }).catch(()=>{});
                }
              }
            }
            await expect(pages[0].locator('#questVotingArea')).toHaveClass(/hidden/, { timeout: 1500 });
          }).toPass({ intervals: [1000, 2000], timeout: 30000 });
      ...
      if (gameRound < 3) {
        ...
        let restartHostPage = null;
        for (const p of pages) {
          if (await p.locator('#btnRestartGame').isVisible()) { restartHostPage = p; break; }
        }

        await expect(async () => {
           if (restartHostPage) {
              await restartHostPage.locator('#btnRestartGame').click({ force: true });
           }
           await expect(pages[0].locator('#waitingArea')).toBeVisible({ timeout: 3000 });
